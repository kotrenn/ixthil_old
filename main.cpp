...
int main(int argc, char **argv)
{
    ...
    TTF_Init();
    Mix_OpenAudio(22050, MIX_DEFAULT_FORMAT, 4, 4096);
    //Note: maybe enable this in OptionsMenu only
    SDL_EnableUNICODE(1);
    SDL_EnableKeyRepeat(SDL_DEFAULT_REPEAT_DELAY, SDL_DEFAULT_REPEAT_INTERVAL);

    int displayFlags = SDL_ANYFORMAT | SDL_HWSURFACE | SDL_DOUBLEBUF;
    ...
    Level *level = new Level(bg, 0, size[0], size[1]);
	...
	while (SDL_PollEvent(&event))
	{
		    ...
		    player->health <= 0)
		{
		    Level *old = level;
		    delete old;
		    level = new Level(bg, 0, size[0], size[1]);
		}
	    }
	    ...
	    player->health > 0)
	{
		...
		if (level->next_level != -1)
		{
			...
			if (old->next_level == 0)
			    ...
			    level = new Level(bg, old->next_level,
					      width, height);
			level->copy(old);
			if (old->subspace)
			    old->exit_subspace();
		...
		{
		    delete level;
		    level = new Level(bg, 0, width, height);
		}
	    }
	    else
		level->random_ships();
	...
	{
	    Level *old = level;
	    player->demo = (old->next_level == 0);
	    if (old->next_level == 0)
		level = new Level(bg, 0, size[0], size[1]);
	    else
		level = new Level(bg, old->next_level, width, height);
	    delete old;
	}
    }
